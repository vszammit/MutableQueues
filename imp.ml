...
let test () : bool =
  join_option (Some (Some 8)) = Some 8
;; run_test "join_option Some option" test
...
let test () : bool =
  cat_option [None; None; None] = []
;; run_test "cat_option list contains all None options" test

let test () : bool =
  cat_option [ Some 1; Some 2; Some 0] = [1;2;0]
;; run_test "cat_option list contains all Some options" test
...
;; run_test "partial_transform none positive squaring" test

let test() : bool =
  let f = fun x -> if x > 0 then Some (x) else None in
  partial_transform f [0; 1; 2; -3] = [1;2]
;; run_test "partial_transform positive " test
(******************************************************************************)
...
let test () : bool =
  let l = [{count = 1}; {count = 2}; {count = 3}] in
  iter (fun r -> r.count <- 3 + r.count) l;
  l = [{count = 4}; {count = 5}; {count = 6}]
;; run_test "iter non-empty list" test
...
let test () : bool =
  ...
  let r2 = { count = 9 } in
  let _ = swap r1 r2 in
  (9, 9) = (r1.count, r2.count)
;; run_test "swap records with same contents" test
...
let rec contains_alias_option (x: 'a) (l: 'a option list) : bool =
  ...
  |Some a :: tl -> x == a || contains_alias_option a tl
  end
