...
module LinkedQueue : QueueInterface.QUEUE = struct
  ...
  *)
  ...
  let from_list (l: 'a list) : 'a queue = 
    let rec loop (l1: 'a list)(q: 'a queue) : 'a queue = 
      begin match l1 with 
      |[] -> q
  ...
  *)
  ...
  let truncate (elt: 'a) (q: 'a queue) : unit =
    ...
    |None->()
    |Some n -> if n.v == elt then (q.tail<-Some n ; n.next <- None)
               else loop n.next  
     end 
    in loop q.head
  ...
  let delete (elt: 'a) (q: 'a queue) : unit =
    ...
    |Some n -> if n.v == elt then 
               ...
               |None -> q.head <- n.next; loop n.next prev
               |Some p -> p.next <- n.next; loop n.next prev
               end
