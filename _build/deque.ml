...
let deque_12 () : int deque =
  let rec n = {v = 1; prev = None; next = None }in 
  let rec m = {v = 1; prev = None; next = Some n }in
  n.prev <- Some m; 
  {head = Some m; tail = Some n} 
...
let invalid_acyclic () : int deque =
  let rec n = {v = 1; prev = None; next = None }in 
  let rec m = {v = 1; prev = None; next = None }in
...
let cycle () : int deque =
  let rec x = {v = 1; prev = None; next = None } in 
  let rec y = {v = 1; prev = None; next = Some x}in
  x.prev <- Some y; 
  x.next <- Some y; 
  {head = None; tail = Some y} 
...
let to_list (q: 'a deque) : 'a list =
  ...
  let rec loop(q: 'a dqnode option) : 'a list = 
  begin match q with 
  |None -> []
...
let insert_head (x: 'a) (q: 'a deque) : unit =
  ...
  let newdqnode = Some {v = x; next= None; prev= None} in
  begin match q.head with 
  |None -> q.head <- newdqnode; q.tail <- newdqnode
  |Some n -> q.head <- newdqnode; n.prev <- newdqnode;
    begin match q.head with
      |Some x -> x.next <- Some n 
      |None -> ()
      end
  end
...
let insert_tail (x: 'a) (q: 'a deque) : unit =
  ...
  let newdqnode = Some {v = x; next = None; prev = None} in
  begin match q.tail with 
  |None -> q.tail <- newdqnode; q.head <- newdqnode
  |Some n -> n.next <- newdqnode; q.tail <- newdqnode;
    begin match q.tail with
      |Some x -> x.prev <- Some n 
      |None -> ()
      end
  end
...
let remove_tail (q: 'a deque) : 'a =
  ...
  begin match q.tail with 
  |None -> failwith "empty deque"
  |Some n -> q.tail <-n.prev;
            begin match n.prev with 
            |None -> q.head <- None; n.v
            |Some x -> x.next <- None; n.v
            end
  end
...
let delete_last (v: 'a) (q: 'a deque) : unit =
  ...
  let rec loop(v:'a)(q1: 'a dqnode option):unit = 
  begin match q1 with
  |None -> ()
  |Some n -> if n.v <> v then loop v n.prev else 
             begin match n.next, n.prev with
             |(Some a, Some b) -> a.prev <- Some b; 
                                    b.next <- Some a;
             |None, Some x -> x.next <- None; q.tail <- Some x 
             |Some y, None -> y.prev <- None; q.head <- Some y 
...
let delete_first (v: 'a) (q: 'a deque) : unit =
  ...
  begin match q1 with
  |None -> ()
  |Some n -> if n.v <> v then loop v n.next else 
             begin match n.next, n.prev with
             |(Some a, Some b) -> a.prev <- Some b; 
                                    b.next <- Some a;
             |None, Some x -> q.tail <- Some x; x.next <- None
             |Some y, None -> q.head <- Some y; y.prev <- None 
             |None, None -> q.tail <- None; q.head <- None
             end
  end
  in loop v q.head
...
let reverse (q: 'a deque) : unit =
  ...
  let rec loop(q1: 'a dqnode option): unit = 
  begin match q1 with 
  |None -> ()
  |Some n -> let temp_n = n.next in 
             let temp_p = n.prev in
             n.next <- temp_p; 
             n.prev <- temp_n;
             if n.next = None then q.tail <- Some n;
             if n.prev = None then q.head <- Some n;
             loop n.prev
  end 
  in loop q.head
