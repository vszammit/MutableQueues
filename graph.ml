...
(* This is a challenge problem, and it is worth no points.  It is fairly
   difficult, but it exercises almost everything we've taught you so
   far.  If you have time to at least take a crack at it, we highly
   recommend doing so!
   
   Here we define directed graphs.  When computer scientists talk
   about graphs, they usually don't mean bar graphs with error lines.
   Instead they tend to mean a network-like diagram, consisting of
   NODES connected by EDGES.
   
   In our graphs, we'll be generic in the "names" used to label nodes:
   we only ever need to compare names for equality, so it shouldn't
...
(* Here's a graph:
 ...
 arrows: a links to c and c links tail to a.
...
(* We define some functions which determine whether a graph is 
   'valid', i.e. it meets these GRAPH INVARIANTS
...
;; run_test "valid invalid_graph" test
...
(* Finish our implementation of "depth-first search".
